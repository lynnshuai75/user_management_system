...
class Auth extends Database {
    ...
    // ***** Forgot Password | updating user with token and token_expire  *****
  ...
  //*********  Reset Password User Auth | checking if user with the emai and token exist ****** */
...
//** verify E-mail of the user */
public function verify_email($email){
    $sql = "UPDATE users SET verify=1 WHERE email=:email AND deleted !=0";
    $stmt = $this->conn->prepare($sql);
    $row = $stmt->execute(['email'=>$email]);
    return true;
}
