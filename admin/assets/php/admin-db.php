...
//** total Verified and unverified users */
...
//** function to handle fetch all users ajax request */
...
public function fetchAllFeedback(){
    $sql = "SELECT feedback.id, feedback.subject, feedback.feedback, feedback.created_at, feedback.uid, users.name, users.email FROM feedback 
       INNER JOIN users ON feedback.uid = users.id WHERE replied !=1 ORDER BY feedback.id DESC";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
...
public  function replyFeedback($uid, $message){
    ...
    $stmt = $this->conn->prepare($sql);
    $stmt->execute(['uid'=>$uid, 'user'=>'user', 'message'=>$message]);
    return true;
}
...
public function fetchNotification(){
    $sql = "SELECT notification.id, notification.message, notification.created_at, users.name, users.email
            FROM notification INNER JOIN users ON notification.uid = users.id WHERE type='admin' ORDER BY
            notification.id DESC LIMIT 7"; 
    $stmt = $this->conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return $result;
...
//** Fetch All Users from Database for export into excel sheet */
